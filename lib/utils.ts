...
export const translate = (title: string, trans: Translations): string => {
  // Manejar rutas de propiedades anidadas como "dashboard.title"
  if (title.includes('.')) {
    ...
    for (const key of keys) {
      if (result && typeof result === 'object' && result.hasOwnProperty(key)) {
        ...
        return title; // Devolver el título original si no se encuentra la ruta
      }
    }
    
    return typeof result === 'string' ? result : title;
  }
  
  const lowercaseTitle = title.toLowerCase();

  if (trans?.hasOwnProperty(lowercaseTitle)) {
    const translation = trans[lowercaseTitle];
    // Si la traducción es un objeto, devolver el título original
